...
const Cart = () => {
  ...
  const total=CartReducer.map((ele)=>ele.qty*(ele.price)).reduce((acc,i)=>acc+i,0)
  // console.log(total);
  ...
  return (
    <div className="container">
        ...
        <div key={ele.id} className="row m-5 border p-3">
          ...
          <div className="col-md-5">
            ...
            <p className="lead fw-bold">
              {ele.qty} X {Math.ceil(ele.price)*70} = ₹{ele.qty * (Math.ceil(ele.price)*70)}
            </p>
            <button onClick={() =>dispatch(delCart(ele))} className="btn btn-outline-dark me-4">
      ...
      <div className="d-flex justify-content-around mb-5">
        ...
        <h3></h3>
        <button className="btn btn-outline-success me-4"><i className="fa fa-credit-card-alt" aria-hidden="true"></i> Proceed To Checkout</button>
      </div>
    </div>
  );
};
